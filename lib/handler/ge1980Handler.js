...
async function insertType({rows, timestamp, connection}){
    for(let row of rows){
        ...
        if(row.year < 1980){
            console.log("INSERT: ROW NOT IN DATA SCOPE", row);
            continue;
        }
        ...
        if(res > row.lastUpdated){
            console.log("INSERT: ROW IS OLD", row);
            continue;
        }
            
        if(res === row.lastUpdated && origin !== 'CENTRAL'){
            console.log("INSERT: CONCURRENT BUT NOT CENTRAL", row);
            continue;
        }
        ...
        try{
            ...
            console.log("INSERT: ERROR IN INSERTING OR UPDATING", row);
            return;
        }
    }
...
async function deleteType({rows, timestamp, connection}){
    for(let row of rows){
        console.log("ATTEMPT", row);
        if(row.year < 1980){
            console.log("DELETE: DATA NOT IN SCOPE", row);
            continue;
        }
        ...
        if(res && res.lastUpdated)
            ...
            res = new Date(0);
        if(res > timeIssued){
            console.log("DELETE: TASK IS OLDER THAN EXISTING DATA", row);
            continue;
        }
        if(res === timeIssued && origin !== 'CENTRAL'){
            console.log("DELETE: CONCURRENT BUT NOT CENTRAL", row);
        }
        try{
            let values = [row.id, row.name, row.year, row.lastUpdated];
            let rankClause = !row.rank ? "IS NULL" : `= ${row.rank}`
            await poolPromise.execute("DELETE FROM `movies` WHERE `id` = ? AND `name` = ? AND `year` = ? AND `rank` " + rankClause + " AND lastUpdated = ?", values);
            ...
            await connection.rollback();
            pool.releaseConnection(connection);
            console.log("DELETE: ERROR IN SQL", row);
            return;
        }
    }
    await connection.commit();
...
async function updateType({rows, timestamp, connection}){
    for(let row of rows){
        ...
        if(res > after.lastUpdated){
            console.log("Unsuccessful UPDATE (DATA IS OLD)", row);
            continue;
        }
            
        if(res === after.lastUpdated && origin !== 'CENTRAL'){
            console.log("Unsuccessful UPDATE (CENTRAL WON)", row);
            continue;
        }

        if(before.year < 1980 && after.year < 1980){
            console.log("Unsuccessful UPDATE (OUT OF SCOPE hard l1980 data)", row);
            continue;
        }
        ...
        if(before.year < 1980 && after.year >= 1980){
            try{
                ...
                await connection.rollback();
                pool.releaseConnection(connection);
                console.log("Unsuccessful UPDATE l -> ge", row);
                return;
            }
            continue;
        }
        ...
        if(before.year >= 1980 && after.year < 1980){
            ...
            await poolPromise.execute("DELETE FROM `movies` WHERE `id` = ? AND `name` = ? AND `year` = ? AND `rank` " + rankClause + " AND lastUpdated = ?", values);
                console.log("Successful UPDATE-DELETE", values);
            }
            catch(e){
                await connection.rollback();
                pool.releaseConnection(connection);
                console.log("Unsuccessful UPDATE ge -> l", row);
                return;
            }
            continue;
        }
        ...
        if(before.year >= 1980 && after.year >= 1980){
            try{
                ...
                await connection.rollback();
                pool.releaseConnection(connection);
                console.log("Unsuccessful UPDATE hard ge1980 data", row);
                return;
            }
        }
    }
