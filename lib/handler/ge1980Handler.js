...
async function updateType({rows, connection}){
    ...
    for(let row of rows){
        ...
        if(before.year < 1980 && after.year < 1980){
            console.log("Unsuccessful UPDATE (OUT OF SCOPE hard l1980 data)");
            continue;
        }
        ...
        let tombstone = await connection.execute("SELECT died FROM graveyard WHERE id = ? FOR UPDATE", [before.id]);
        tombstone = tombstone[0][0];
        tombstone = tombstone ? new Date(tombstone.died) : null;
        ...
        if(before.year < 1980 && after.year >= 1980){
            try{
                if(emptyRes){
                    ...
                    if(tombstone && tombstone > after.lastUpdated){
                        console.log("UPDATE-INSERT: Unsuccessful, necromancy detected...");
                        await connection.rollback();
                        continue;
                    }
        ...
        if(before.year >= 1980 && after.year < 1980){
            try{
                ...
                if(tombstone){
                    await connection.execute("UPDATE graveyard SET died = ? WHERE id = ?", [timeIssued, before.id]);
                }
                else{
                    await connection.execute("INSERT INTO graveyard(id, died) VALUES (?,?)", [before.id, timeIssued]);
                }
        ...
        if(before.year >= 1980 && after.year >= 1980){
            try{
                ...
                await connection.rollback();
                console.log("Unsuccessful UPDATE hard ge1980 data");
                continue;
            }
        }
    }
