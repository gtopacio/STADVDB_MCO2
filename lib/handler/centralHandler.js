...
async function insertType({rows, origin, connection}){
    ...
    for(let row of rows){
        ...
        try{
            ...
            console.log(`ID: ${row.id}, New Clock: ${JSON.stringify(newClock.clock)}, Last Update: ${JSON.stringify(recordedClock.clock)}`);
...
async function updateType({rows, origin, connection}){
    ...
    for(let row of rows){
        ...
        try{
            ...
            console.log("RECORDED Clock: ", recordedClock);
            ...
            if(!storedRecord){
                let values = [after.id, after.name, after.year, after.rank, after.CENTRAL, after.L1980, after.GE1980, newTombstone];
