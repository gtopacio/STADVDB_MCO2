...
async function handler(parsedMessage) {
    ...
    if(type === "insert"){
        await insertType({rows, timestamp, mergedClock, origin, connection});
        return;
    }
    ...
    //         return row;
    //     });
    //     await deleteType({rows, timestamp, mergedClock, origin, connection});
    //     return;
    // }

    if(type === "update"){
        await updateType({rows, timestamp, mergedClock, origin, connection});
        return;
    }
}
