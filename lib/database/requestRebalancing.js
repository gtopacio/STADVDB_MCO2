...
async function requestRebalancing(){
    ...
    try{
        ...
        L1980.releaseConnection(connL1980);
        GE1980.releaseConnection(connGE1980);
...
async function getInvalidRows({connGE1980, connL1980}){
    try{
        let [GEInvalid] = await connGE1980.execute("SELECT *, lastUpdated FROM movies WHERE `year` < 1980;");
        let [LInvalid] = await  connL1980.execute("SELECT *, lastUpdated FROM movies WHERE `year` >= 1980;");
        return {
            GE1980: GEInvalid,
            L1980: LInvalid
        }
...
async function deleteInvalidRows({connL1980, connGE1980}){
    try{
        await Promise.all([
            connL1980.execute("DELETE FROM movies WHERE `year` >= 1980;"),
            connGE1980.execute("DELETE FROM movies WHERE `year` < 1980;")
        ]);
    }
    catch(e){
        throw e;
...
async function insertNewRows({connGE1980, connL1980, invalidRows}){
    try{
        ...
        for(let value of L1980){
            let values = [value.id, value.name, value.year, value.rank, value.lastUpdated];
            await connGE1980.execute("INSERT INTO movies (id, `name`, `year`, `rank`, lastUpdated) VALUES (?,?,?,?,?)", values);
        }
        for(let value of GE1980){
            let values = [value.id, value.name, value.year, value.rank, value.lastUpdated];
            await connL1980.execute("INSERT INTO movies (id, `name`, `year`, `rank`, lastUpdated) VALUES (?,?,?,?,?)", values);
        }
    }
    catch(e){
        throw e;
