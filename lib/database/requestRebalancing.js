...
async function requestTableLock({connGE1980, connL1980}){
    try{
        if(NODE_NAME === "L1980"){
            await connL1980.query("LOCK TABLE movies WRITE");
            return;
        }
        await connGE1980.query("LOCK TABLE movies WRITE;");
    }
    catch(e){
        throw e;
    }
...
async function insertNewRows({connGE1980, connL1980, invalidRows}){
    try{
        ...
        for(let value of L1980){
            ...
            await connL1980.execute("INSERT INTO movies (id, `name`, `year`, `rank`, CENTRAL, L1980, GE1980) VALUES (?,?,?,?,?,?,?) AS new ON DUPLICATE KEY UPDATE id = new.id, `name` = new.`name`, `year` = new.`year`, `rank` = new.`rank`, CENTRAL = new.CENTRAL, GE1980 = new.GE1980, L1980 = new.L1980, tombstone = false", values);
        }
        for(let value of GE1980){
            let values = [value.id, value.name, value.year, value.rank, value.CENTRAL, value.L1980, value.GE1980];
            await connGE1980.execute("INSERT INTO movies (id, `name`, `year`, `rank`, CENTRAL, L1980, GE1980) VALUES (?,?,?,?,?,?,?) AS new ON DUPLICATE KEY UPDATE id = new.id, `name` = new.`name`, `year` = new.`year`, `rank` = new.`rank`, CENTRAL = new.CENTRAL, GE1980 = new.GE1980, L1980 = new.L1980, tombstone = false", values);
        }
    }
    catch(e){
        throw e;
...
async function releaseTableLocks({connGE1980, connL1980}){
    try{
        if(NODE_NAME === "L1980"){
            await connL1980.query("UNLOCK TABLES;");
            return;
        }
        return await connGE1980.query("UNLOCK TABLES;");
    }
    catch(e){
        throw e;
    }
