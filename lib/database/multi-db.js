...
async function dropTempTable({connGE1980, connL1980}){
    try{
        let dropSQL = `DROP TEMPORARY TABLE IF EXISTS ${tempTable}`;
        await Promise.all([
...
async function generateTempTables({connGE1980, connL1980}){
    try{
        ...
        let insertSQL = "INSERT INTO " + tempName + " (id, `name`, `year`, `rank`, lastUpdated) SELECT id, `name`, `year`, `rank`, lastUpdated FROM movies;"
        let promises = [
